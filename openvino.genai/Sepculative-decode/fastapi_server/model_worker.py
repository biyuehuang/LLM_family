...
class ModelWorker:
    def __init__(self, checkpoint, assistant_checkpoint, device="GPU"):
        self.device = device
        start = time.perf_counter()

        draft_model = openvino_genai.draft_model(assistant_checkpoint, device)
        self.model = openvino_genai.LLMPipeline(checkpoint, device, draft_model=draft_model)      
        self.config = openvino_genai.GenerationConfig()
        self.config.max_new_tokens = 2048
        self.config.num_assistant_tokens = 5
